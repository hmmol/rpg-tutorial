...
Creature::Creature(std::string id, JsonBox::Value& v, EntityManager* mgr) : Creature(id, "", 0, 0, 0, 0, 0)
{
	this->load(v, mgr);
}
...
int Creature::traverse(Door* door)
{
	...
	else if (door->locked > 0)
	{
		...
		// Creature does not have the key, so door remains locked
		else
		{
			return 0;
		}
...
// Note that in 'x in range [a, b]' in the comments means that
// a <= x <= b. this is interval notation. 
