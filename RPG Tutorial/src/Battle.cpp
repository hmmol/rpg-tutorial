...
Battle::Battle(std::vector<Creature*>& combatants)
{
	...
	// creatures have not already been assigned unqiue names, 
	...
	for (auto com : this->combatants)
	{
		...
		// If there is already one creature, record that there
		// are more than one. We don't want the actual number, simply
...
{
	// Queue of the battle events. Fastest combatants will be
	...
	// Iterate over the combatants and decide what they do,
	...
	for (auto com : this->combatants)
	{
		if (com->id == "player")
		{
			...
			// Created every turn because someone may die
	...
	while (!events.empty())
	{
		// Take the event from the front of the queue
		...
		case BattleEventType::ATTACK:
		{
			...
			// must check that they're still valid first
...
{
	// Find the creature's position in the combat vector
	auto pos = std::find(this->combatants.begin(), this->combatants.end(), creature);

	// Don't try to delete the creature if it doesn't exist.
	if (pos != this->combatants.end())
	{
		...
		// ensure that the creature's health is 0 and marked as dead.
		creature->hp = 0;
		// Remove the creature
